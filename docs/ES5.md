...
本文档初衷是在项目管理中书写 `JavaScript` 时，能够保持代码风格保持一致，在团队管理中能够易理解和已维护。

本文档适用于在项目中使用各种 `JavaScript` 的预编译语言（如 `TypeScript` 等）。
...
UTF-8 编码具有更广泛的预见。BOM 在使用程序或工具处理文件时可能会造成不必要的干扰。
...
##### [强制] 使用 4 个空格做一个缩进层级，不允许使用 `2` 个空格或 `tab` 字符。
...
##### [强制] 二元运算符两侧必须有一个空格，一元运算符与操作对象直降不允许有空格。
...
##### [强制] 在函数调用、函数声明、括号表达式，属性访问、  `if / for / while / switch / catch` 等语句中，`()` 和 `[]` 内紧贴括号部分不允许有括号
...
##### [强制] 单行声明的数组和对象，如果包含元素， `{}` 和 `[]` 内贴紧括号部分不允许包含空格
...
超过的不可分割的代码允许例外，比如复杂的正则表达式。长字符串不在例外之列
...
##### [建议] `if...else...` 、`try...catch...finally` 等语句，建议使用在 `}` 后添加一个换行，使代码层次结构更清晰，可读性更好。
...
##### [强制] 在 `if / else / do / while` 语句中，即使只有一行，也不得省略代码块 `{....}`。
...
##### [强制] `变量` 、 `函数` 、`函数的参数`、class类或构造函数的 `方法 / 属性`、`命名空间` 等使用 `小驼峰(Camel)` 命名法。
...
##### [强制] `构造函数` 和  `class类` 使用 `大驼峰(Pascal)` 命名法。
...
##### [强制] `枚举变量` 使用 `大驼峰(Pascal)` 命名法，`枚举的属性` 使用 `全部字母大写，单词间下划线分隔` 的命名方式。
...
##### [强制] 为了便于阅读和自文档化，以下内容必须包含以 `/**...*/` 形式的块注释中。
...
类型不仅局限于内置类型，也可以是自定义类型。比如定义了一个类 `Developer`，就可以使用它来定义一个参数和返回值的类型。
...
| 不允许为null | {!Object} | Object 类型，但不是null |
...
对于内部实现、不容易理解的说明、摘要信息等，我们需要编写细节注释。
...
不通过 `var / const / let` 定义变量将导致变量污染全局环境。
...
原则上不建议使用全局变量，对于已有的全局变量或第三方框架引入的全局变量，需根据检查工具的语言标识。
...
##### [强制] 变量必须 `即用即声明`，不得在函数或其他形式的代码块起始位置统一说明所有变量。
...
##### [建议] 对于相同变量或者表达式的多值条件，用 `switch` 代替 `if`。
...
##### [建议] 复杂的数据到视图字符串的转换过程，选用一种模版引擎。

解释：

使用模版引擎有如下好处：

1. 在开发过程中专注于数据，将试图生成的过程有另外一个层级维护，使程序逻辑结构更清晰。
2. 优秀上午模版引擎，通过模板编译技术和高质量的编译产物，能获得比手工拼接字符串更高的性能。
3. 模版引擎能方便的对动态数据进行相应的转义，部分模版引擎默认进行HTML转义，安全性更好。
...
##### [建议] 对象创建时，如果任何一个 `属性` 需要添加引号，则所有 `属性` 建议添加引号。
...
通常在 `JavaScript` 中声明的对象，属性命名是使用 `Camel` 命名法，用 `.` 来访问更清晰简洁。部分特殊的属性（比如来自后端JSON），可能采用不寻常的命名方式，可以通过 `[expr]` 方式访问。
...
将过多的逻辑单元混在一个大函数中，易导致难以维护。一个清晰易懂的函数应该完成单一的逻辑单元。复杂的操作应进一步抽取，通过函数调用来体现流程。
...
除去不定长参数以外，函数具备不同逻辑意义的参数建议控制在 `6` 个以内，过多的参数会导致维护难度增大。

某些情况下，如使用 AMD Loader 的 `require` 加载多个模块，其 `callback` 可能会存在较多参数，因此对函数参数的个数不做强制限制。
...
- 当配置项有增长时，无需无休止地增加参数个数，不会出现 `removeElement(element, true, fakse, false, 3)` 这样难以理解的调用代码。
- 当部分配置参数可选时，多个参数的形式非常难以处理重载逻辑，而使用一个 `options` 对象只需判断属性是否存在，实现得以简化。
...
- 是否使用了 with 表达式。
 ...
 Chakra、V8 和 SpiderMonkey 将受以上因素的影响，表现出不尽相同又较为相同的回收策略，而 JScript.dll 和 Carakan 则完成没有这方面的优化，会完整保留整个 LexcalEnvironment 中所有变量绑定，造成一定的内存消耗。

 由于对闭包内变量有优化策略的 Chakra、V8 和 SpiderMonkey 引擎的行为较为相似，因此可以总结如下，当返回一个函数 fn 时：
...
JavaScript 因其脚本语言的动态特性，当一个对象未被 seal 或 freeze 时，可以任意添加、修改，删除属性值。

但是随意地对非自身控制的对象进行修改，很容易造成代码在不可预知的情况下出现问题。因此设计良好的组件、函数应该避免对外部传入的对象的修改。

下面代码的 **selectNode** 方法修改了有外部传入的 **datasource** 对象。如果 **datasource** 用在其他场合（如另一个 Tree 实例）下，会造成状态的混乱。
...
由于此类场景，需要使用额外的对象来维护，使用由自身控制，不与外界产生任何交互的 **selectedNodeIndex** 对象来维护节点的选中状态，不对 **datasource** 做任何修改。
...
AMD 作为由社区认可的模块定义形式，提供多种重载提供灵活的使用方式，并且绝大多数优秀的 Library 都支持AMD，适合作为规范。
...
在 AMD 的设计思想里，模块名称是所在路径相关，匿名的模块便利与封包和迁移。模块依赖应在模块定义内部通过 `local require` 引用。
...
使用 return 可以减少 factory 接收的参数（不需要接收 exports 和 module），在没有 AMD Loader 的场景下也容易进行简单的处理来伪造一个 Loader。
...
##### [强制] 全局运行环境，`require` 必须以 `async require` 形式调用。
...
2. 在 AMD 里，`global require` 是可以被重命名的。并且 Loader 甚至没有全局的 `require` 变量，而是用 Loader 名称作为 `global require`。模块定义不应该依赖使用的 Loader。
...
对于任何可能通过 发布-引入 的形式复用第三方库、框架、包、开发者所定义的名称不代表使用者使用的名称。因此不要基于任何名称的假设。在实现源码中，`require` 自身的其他模块时使用 `relative id`。
...
##### [建议] 对于单个元素，尽可能使用 `document.getElementById` 获取，避免使用 `document.add`。

##### [建议] 对于多个元素的集合，尽可能使用 context.getElementByTagName 获取。其中 context 可以为 document 或其他元素。指定 tagName 参数为 * 可以获得所有子元素。

##### [建议] 遍历元素集合时，尽量缓存集合长度。如需多次操作同一集合，则应将集合转为数组。

解释：

原生获取元素结合的结果并不直接引用 DOM 元素，而是对索引进行引用，所以 DOM 结构的改变会实时反映到结果中。
